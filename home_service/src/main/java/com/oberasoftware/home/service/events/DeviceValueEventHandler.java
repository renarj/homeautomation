...
public class DeviceValueEventHandler implements EventHandler {
    ...
    public void receive(DeviceValueEvent event) {
        ...
        DeviceItem item = deviceManager.findDeviceItem(event.getControllerId(), event.getPluginId(), event.getDeviceId());

        stateManager.updateState(item.getId(), event.getLabel(), event.getValue());
    }
}
